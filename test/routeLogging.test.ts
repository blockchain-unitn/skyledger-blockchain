...
describe("RouteLogging", function () {
  ...
  it("should revert getLogsOfDronePaginated with invalid limit", async function () {
    await expect(routeLogging.getLogsOfDronePaginated(1, 0, 0)).to.be.reverted;
    await expect(routeLogging.getLogsOfDronePaginated(1, 0, 101)).to.be
      .reverted;
  });
  ...
  it("should return all unique drones authorized by a UTM", async function () {
    ...
    const dronesByOwner = await routeLogging.getDronesAuthorizedByUTM(
      owner.address
    );
    expect(dronesByOwner.map((id: any) => Number(id)).sort()).to.deep.equal([
      1, 2,
    ]);
    const dronesByAddr1 = await routeLogging.getDronesAuthorizedByUTM(
      addr1.address
    );
  ...
  it("should revert when getting zones of a log that does not exist", async function () {
    await expect(routeLogging.getZonesOfLog(99)).to.be.reverted;
  });

  it("should revert when getting a log that does not exist", async function () {
    await expect(routeLogging.getLog(42)).to.be.reverted;
  });

  it("should revert when getting zones of a non-existent log", async function () {
    await expect(routeLogging.getZonesOfLog(42)).to.be.reverted;
  });
  ...
  it("should handle safe UTM drone query with limits", async function () {
    ...
    const [drones, hasMore] = await routeLogging.getDronesAuthorizedByUTMSafe(
      owner.address,
      5
    );
    ...
    await expect(routeLogging.getDronesAuthorizedByUTMSafe(owner.address, 0)).to
      .be.reverted;

    await expect(routeLogging.getDronesAuthorizedByUTMSafe(owner.address, 51))
      .to.be.reverted;
  });
});
